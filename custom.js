...
$("#copy-embed-btn").click(function(){
  $("textarea").select();
  document.execCommand('copy');
  document.getElementById('clipboard-message').innerHTML = 'Embed copied to clipboard!';
  $('#clipboard-message').toggleClass('d-block');
});
...
$('#generate-btn').click(function() {
  ...
  const rssUrl = $('#rss-url').val();

  if (siteUrl == '' || rssUrl == '' ) {
    const msg = 'Both fields must be filled out.';
    createAlert(msg);
  } else if (!isUrlValid(siteUrl) || !isUrlValid(siteUrl)) {
    const msg = 'You must enter a valid URL.';
    createAlert(msg);
  } else {
    $('#copy-embed-btn').removeClass('disabled');
    ...
    const generatedCode = `
      ...
      <div id="podcasts-player-container">
        <iframe id="anchor-podcast-iframe" src="${siteUrl}/embed" style="width: 100%;" frameborder="0" scrolling="no" name="iframe"></iframe>
    ...
    $('#generated-code').val(generatedCode);
  }
